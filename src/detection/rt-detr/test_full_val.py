...
RT-DETR 학습된 가중치로 val(=test) 셋 전체를 추론해서, 박스+클래스+confidence가
그려진 이미지를 저장한다 (experiment/rt-detr 브랜치 전용).

combined_10ep 실험(2차 실험, 클래스당 1장씩 샘플만 확인)과 달리, 이 스크립트는
datasets/rtdetr_combined/images/val 전체(99장)를 대상으로 한다.

사용법:
    python src/detection/rt-detr/test_full_val.py
    python src/detection/rt-detr/test_full_val.py --weights runs/rtdetr/combined_run/weights/best.pt
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="RT-DETR val셋 전체 추론 테스트")
    parser.add_argument("--images", default=str(REPO_ROOT / "datasets" / "rtdetr_combined" / "images" / "val"))
    parser.add_argument("--weights", default=str(REPO_ROOT / "runs" / "rtdetr" / "combined_run" / "weights" / "last.pt"))
    parser.add_argument("--output", default=str(REPO_ROOT / "runs" / "rtdetr_test" / "full_val"))
